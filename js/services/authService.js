...
export class AuthService {
  sendCode(email, purpose = 'login') {
    ...
    if (!e || !/[^\s@]+@[^\s@]+\.[^\s@]+$/.test(e) && !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(e)) {
  ...
  async verifyCode(email, code, purpose, profile = {}) {
    ...
    if (!record.isValid) return { ok: false, message: 'Код истёк. Запросите новый.' };
    ...
    if (!psy) return { ok: false, message: 'Психолог не найден' };
    ...
    if (!unlocked) return { ok: false, message: 'Неверный пароль' };
