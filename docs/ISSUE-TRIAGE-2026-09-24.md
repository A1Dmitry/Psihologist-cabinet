...
> **Контекст:** триаж выполнен против main @ `d578c08`. Актуальный main сменился
> (PR #32, #37 → `87e3951`); актуальное состояние — [`CURRENT-STATE.md`](CURRENT-STATE.md).
> Готовые комментарии по #7/#11/#15/#8/#14 остаются исполнимыми; статусы ишью
> перед публикацией сверить с CURRENT-STATE.md.

> **Текущая политика входа (добавлена позднее):** новые коды запрашиваются только через `auth-code`; GoTrue `/auth/v1/otp` не является fallback ни при 404, ни при status=0/CORS/сети. Сохранённый legacy pending OTP можно завершить только по исходному каналу. См. `docs/CURRENT-STATE.md` и SR-006.
...
## Инструмент для владельца: production E2E одним запуском (добавлено 2026-09-24)

`tools/prod-e2e.mjs` — исполняет настоящий use case регистрации на production с машины владельца:

```bash
node tools/prod-e2e.mjs                          # диагностика сервера (письма не шлёт)
node tools/prod-e2e.mjs --email <тестовый@email> # полный E2E: код вводится из письма
```

20 проверок: запрос кода → письмо → Auth-сессия → claim → поля профиля → reload →
повторный вход без дубля → неверный/повторный код. Секретов не требует; код/токены
не печатаются. Порядок такой: пункты выше (секреты CI, schema, Resend) → этот запуск →
вывод в issue #14 → закрытие задачи по факту.

Попутно исправлен дефект диагностики: `serverDiagnostics()` вызывал
`create_booking`/`claim_psychologist_profile` с пустым `{}` — на исправном проде
PostgREST отвечал 404 (обязательные аргументы) и давал ложные ⛔. Теперь пробы
детерминированные (2xx-ответы функций на «пустые» данные).
