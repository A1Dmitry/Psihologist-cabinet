...
**Статус закрытия:** ишью готово к закрытию владельцем. У App-токена сессии нет
`issues:write` (подтверждено HTTP 403 на `PATCH /issues/36` и на создании
комментария), а PR #60 с keyword `Closes #36`, смерженный этим же App, не
триггернул системный auto-close: GitHub закрывает ишью по keyword, только если
у актора merge есть права записи на ишью. Все сегодняшние закрытия в репозитории
(#21, #22, #34, #51, #54) выполнены `closed_by=A1Dmitry` — это каноничный путь
и для #36: весь требуемый DoD-evidence — на `main`, осталось действие владельца.
