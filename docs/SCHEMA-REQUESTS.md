...
- **Статус:** `applied` 2026-09-23 (AUDIT-REG-DRY-001). Итоговый контракт: `sessions.client_timezone text`
  (IANA) + `sessions.client_utc_offset_min integer` (снимок, check −840…840) +
  `sessions.duration_min integer` (снимок длительности). Ранняя колонка
  `sessions.timezone_offset text` ('+02:00') удалена как альтернативное имя того же
  бизнес-поля в несовместимом формате. `scheduled_at timestamptz` НЕ добавлен —
  остаётся отдельной заявкой (см. follow-up в `docs/FULL-AUDIT-REPORT.md`).
...
> **Замечание (AUDIT-REG-DRY-001, 2026-09-23):** правило диапазонов задним числом
> нарушено — SR-001 и SR-002 описаны дважды (Агент 1 «часовой пояс в `sessions`» /
> «Google Meet» выше по файлу и Агент 2 «часовой пояс в записи» / «интервал в
> `create_booking`» в конце). Дубли по смыслу: SR-001(Агент 1) ≈ SR-001(Агент 2) ≈ SR-108 —
> все три про `sessions.client_timezone`; закрыты одним контрактом, новых колонок не
> добавлено. SR-002(Агент 1, Google Meet) остаётся открытой и не пересекается с
> SR-002(Агент 2). Новые заявки — строго в свободный номер своего диапазона; старые
> номера не переиспользовать.
...
| SR-005 | 2026-09-24 | Агент 2 (публичная запись) | Google-authenticated triage booking: связывать `sessions` с `auth.uid()` и запрещать прикрепление опроса без Google-подтверждения | `schema.sql` → `sessions` + RPC `create_booking`; owner-only чтение | блокер для серверного enforcement; client UI реализован | ждёт Агента 1 и production config; issue #41 |
| SR-006 | 2026-09-24 | психолог auth (#40; signup-link + invitation claim) | Активная запись `psychologists` как allowlist; подтверждение email Supabase при первичной регистрации; отдельный одноразовый хешированный код привязывает `auth.uid()` только к существующему профилю | `schema.sql` + private invitation table/RPC; `auth-code` gate; callback/Pages integration | блокер безопасной регистрации психолога и выдачи сессий | ждёт Агента 1, безопасности invite/bind и отдельного решения по Supabase Auth password; issue #40 |
| **SR-007** | 2026-09-25 | Агент 2 (публичная запись/вход) | **Открытая регистрация специалиста через Google:** `psychologists.profile_completed`; RPC `link_or_create_psychologist_for_google()` (атомарный поиск→привязка→создание, identity только из `auth.users`) и `complete_psychologist_profile()`; запрет прямого INSERT и колоночные гранты UPDATE для `authenticated` (email/is_active/owner_id/profile_completed защищены); `public_profiles` скрывает недозаполненные кабинеты | `supabase/migrations/20260925_google_specialist_signup.sql` (НЕ применена), `js/services/googleAuthService.js`, `js/services/supabaseApi.js`, `index.html`, `js/app.js` | блокер входа через Google на проде | **⏳ миграция подготовлена и проверена на PostgreSQL (66 проверок), НЕ применена владельцем** — требуется ручное применение + включение провайдера Google в Supabase. См. `docs/GOOGLE-SPECIALIST-SIGNUP.md` |
...
- **Статус:** `applied` 2026-09-23 (AUDIT-REG-DRY-001). Сверено с заявкой Агента 2 по T-03:
  это то же самое поле, дубль НЕ создавался. Итоговое имя — `client_timezone` (+ снимок
  `client_utc_offset_min`). Маппинг в `cabinetApi.js` — через канонический
  `js/services/sessionMapper.js`, а не локальной копией.
...
- **Статус:** `applied` 2026-09-23 (AUDIT-REG-DRY-001). Итоговые имена: `client_timezone text`
  + `client_utc_offset_min integer` (вместо `client_utc_offset` — единое имя со стороны SR-108)
  + `duration_min integer`. Параметры RPC: `p_client_timezone`, `p_client_utc_offset_min`,
  `p_duration_min`. Строка-хак в `sessions.note` удалён. В проде требуется переприменение
  `schema.sql` (п.2 `docs/INFRA.md`).
...
- **Статус:** `applied` 2026-09-23 (AUDIT-REG-DRY-001), с расширением: (1) интервальный
  overlap для `sessions` **и** для `schedule_blocks`; (2) гонка закрыта серверно через
  `pg_advisory_xact_lock(hashtext('booking:'||psychologist_id||':'||session_date))` —
  проверка и INSERT теперь в одной транзакции; (3) RPC возвращает `{ok, client_id,
  session_id, duration_min}`. Сигнатура изменена (22 параметра), гранты
  `revoke`/`grant execute` перегенерированы — до этого они указывали старую арность и
  весь файл `schema.sql` падал на 42883. Проверено на реальном PostgreSQL 18.4:
  `node tests/db-contract.mjs` → 36/36 PASS.
...
- **Приоритет:** обычный.
- **Статус:** `applied` 2026-09-23 (AUDIT-REG-DRY-001). Уточнение: view уже содержал
  `duration_min` (`coalesce(s.duration_min, sv.duration_min, ss.slot_step_min, 60)`), но
  **никто не читал** колонку — `listBookedSlots` её не запрашивал, а
  `BookingViewModel._busyIntervalsFor` считал занятость по шагу сетки. Исправлено на
  стороне клиента: `listBookedSlots` выбирает `duration_min`,
