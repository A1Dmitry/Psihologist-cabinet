...
- `create_booking` **недоступен роли anon** в production. По `schema.sql` EXECUTE
  выдаётся anon (публичная страница записи пишет именно так), следовательно
  публичная запись в проде **не работает**. Два неразличимых anon-каналом
  объяснения: функции нет вовсе **или** она существует в старой арности, а
  `grant execute` исторически падал на неверной сигнатуре (задокументированный
  дефект 42883 в `docs/INFRA.md`). Различает только SQL-канал владельца
  (`docs/OWNER-CHECKLIST-E2E.md`, Блок 7).
