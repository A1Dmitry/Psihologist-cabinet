...
- **T-02 · Слоты подстраиваются под услугу (S)** — длительность услуги (60/90 мин) режет
  сетку: окно занято, если до следующего слота не хватает времени. Точка: `BookingViewModel.slots`.
  Приёмка: для 90-мин услуги слоты 18:00 при конце дня 19:00 недоступны.
- **T-03 · Часовой пояс клиента (M)** — показывать время в поясе клиента
  ...
  Точки: `BookingViewModel`, `sessions` (поле `timezone_offset`), schema.sql.
...
- **T-16 · Уведомления психологу (S)** — email/Telegram психологу о новой записи,
  переносе, оплате (сейчас клиентские механики только в demo-режиме).
...
- **T-22 · Автосоздание видеоссылки на сессию (M)** — генерация Meet-ссылки через
...
## 4. Открытые вопросы (нужны решения владельца продукта)

1. Видеосессии: достаточно автосоздания Google Meet-ссылок (T-22) или нужно встроенное
   видео внутри портала (как у ОКОН, существенно дороже)?
2. Платежи: какие провайдеры приоритетны? bePaid есть; Stripe/PayPal нужны для клиентов
   вне Беларуси?
3. Telegram-уведомления: готов ли психолог поддерживать своего бота (токен)?
4. Кабинет клиента (T-12): ссылка-токен (как у ОКОН, без регистрации) — подтверждается?
5. Часовой пояс портала по умолчанию — Europe/Minsk; нужны ли несколько языков интерфейса?
