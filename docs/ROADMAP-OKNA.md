...
- **T-03 · Часовой пояс клиента (M)** — ✅ показывать время в поясе клиента
  ...
  конверсия слотов при записи из другого пояса (в БД пишется время специалиста;
  пояс клиента — в заметке до SR-001).
  Точки: `BookingViewModel`, `calendarService` (утилиты поясов), `sessions` (поле — SR-001).
- **T-04 · «Выберите услугу — покажем окна» (S)** — ✅ клик по услуге сразу ведёт к
  свободным дням/окнам этой услуги (шаг 2), с профиля — ссылка `/book/{slug}?service=<id>`.
  Точка: `renderBooking`, `renderProfile`.
