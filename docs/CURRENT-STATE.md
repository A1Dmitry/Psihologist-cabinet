...
- **production UI без demo (#121, R14)** — в ветке `arena/01a0df81` (`1ffcc6e`, ждёт подтверждения
  ...
  в кабинете снята симуляция отправки напоминаний (`1f7f654`) — `tests/production-ui-no-demo.mjs`
  (40, A12 — свип всего продукта на «демо»), отчёт `docs/ISSUE-121-REPORT.md`;
...
| #63 | P0 | Мобильный кабинет 1.0 (bottom tab bar, 15 разделов, карточные действия) — **реализовано в main** (PR #101, merge `8c96311`; Main Re-Audit PASS — `docs/MAIN-AUDIT-63.md`; Challenger — `docs/ISSUE-63-CHALLENGER.md`, PR #103 → `c6c2f2e`). **Ре-аудит на `c6c2f2e` (2026-09-26, `EXEC-rzVmS30TVl`) нашёл и устранил 3 дефекта контура**: панель «…» не закрывалась после тапа по пункту без перерендера; `.cab-menu{z-index:30}` рисовалась под фиксированным таб-баром (`z-index:40`) — деструктивные пункты недоступны тапу; ложная посылка/постороннее имя в комментарии контроллера. Негативный контроль (4 FAIL до фикса, воспроизведён независимо на baseline-дереве), гейт 40/40 **1809** проверок, смоук `verify_pages.py` 13/13, мутационная матрица QA 4/4 мутанта убито — `docs/ISSUE-63-REAUDIT.md` (+ раздел «QA-ПРОГОН»). **PR #104 — MERGED** в `main @ f087642` (2026-09-26T18:25:29Z); Main Re-Audit на merge-SHA — PASS (`docs/MAIN-AUDIT-63B.md`): снимок `git archive origin/main`, гейт 40/40 **1809**, смоук 13/13; внешний CI: `Deploy to GitHub Pages` success, шаг `Smoke check deployed site` success, `Rebuild Tailwind CSS` success (в прод ушёл `z-index:45`). Остаток: независимый Challenger правок (§6.6) + прогон мобильной раскладки владельцем на устройстве → закрытие; CLAIM-маркер в Issue не публикуется (нет `issues:write`) |
...
| #121 | P2 (R14) | Production UI без demo-записи/входа email+пароль/сброса — **сделано в ветке** `arena/01a0df81-psihologist-cabinet` (`1ffcc6e` публичная страница + `1f7f654` кабинет по решению владельца «больше никаких демо»: симуляция «Отправить due сейчас»/«Демо-исходящие»/«Симулировать ответ» и `card_demo` сняты). Гейт 41/41 **1855**, смоук 13/13, fail-before-fix 28 FAIL → ALL PASS (40), свип продукта A12 = 0 упоминаний демо. Осталось: merge в `main` (владелец), Challenger/Main Re-Audit — OPEN; CLAIM-маркер и закрытие Issue интеграцией — нет `issues:write`. Границы без демо-масок: клиент в локальном зеркале после отказа сервера — R01/R02; серверный lifecycle напоминаний/ответа по ссылке — R09 |
