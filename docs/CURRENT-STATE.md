...
## Главная открытая задача (зарегистрирована 2026-09-24)

**`docs/TASK-P0-SUPABASE-PORTAL.md` — P0 / наивысший приоритет владельца.**
Безопасный портал психологов: email-redirect на опубликованное приложение
(не localhost), одноразовая серверная привязка Auth-аккаунта к существующей
записи психолога по хешированному коду, RLS-изоляция (свой профиль + свои
записи; второй психолог / `anon` / непривязанный `authenticated` — ноль строк).

Статус: **спецификация сохранена, реализация НЕ начата** — открыты блокирующие
параметры (раздел 4 того файла: production `app_url`, фактический callback-маршрут,
состояние живой схемы, перечень «запросов»). Это согласуется с разделом
«Production: что доведено (честно)» ниже: schema/APP_URL/Site URL в проде — UNKNOWN.
