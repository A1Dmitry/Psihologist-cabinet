...
4. ~~Probe issue #49~~ — **ЗАКРЫТ владельцем** 2026-09-25T05:31:58Z (без комментариев). Трекер чист.
...
## 9. Merge PR #48 — Main Re-Audit нового main (2026-09-25)

Владелец влил PR #48 (merged 2026-09-25T05:31:29Z) → `origin/main = ca3b23b2d964402083c49b88552d1c25cec6a08f`.
Дифф мержа — ровно 8 файлов PR #48 (164+/4-), постороннего нет.

Main Re-Audit по RULES §6.7 (проверен актуальный `main`, не ветка; код взят через
`git archive ca3b23b` в `/tmp/maincheck`, рабочая ветка не переключалась):

- **Гейт: 24/24, exit 0** (`node tools/verify_all.mjs`); **pages: ALL PASS, exit 0**
  (devserver на :8766). Merge не сломал main.
- **Дефект F5 закрыт**: `#auth-code-hint` есть в `index.html` (1 вхождение);
  `verificationHint(channel, email)` экспортирован (`registration.js:230`);
  `get codeHint()` в `AuthViewModel:42`; запасной канал честно пишет
  «в нём ссылка, а не код».
- **Соседние сценарии**: auth-контракт (`registration-flow`, `auth-code-edge`,
  `auth-ui-pending`, `verify_auth`) — все зелёные; регрессий нет.
- **Состояния GitHub**: 11 closed (включая #15 и #49), 13 open
  (#21/#22/#27/#28/#29/#31/#34/#35/#36/#40/#41/#46/#50). Открытых PR нет.
- **Следствие для #50**: current-state документы теперь отстают на два merge
  (утверждают `dcb4093`, факт `ca3b23b`); scope #50 покрывает и этот merge.

---

## 10. Пересмотр закрываемости + Challenger #36 + фикс #50-частичный (2026-09-25)

### 10.1 Какие ишью можно закрыть (все 13 открытых перепроверены)

| Issue | Можно закрыть? | Основание |
|---|---|---|
| #21, #22 | Нет | #21 п.4 (`completePayment` local-only) жив в main; production-гейты обоих BLOCKED |
| #27/#28/#29/#31 | Нет | BA-backlog, не стартовали (Stop-the-Line #46) |
| #34 | Нет | Нужны свежий Challenger + Main Re-Audit на `ca3b23b` + production-верификация |
| #35 | Нет | LIVE: функции не задеплоены; свежего среза нет (dispatch probe из сессии — 403) |
| **#36** | **Да (владельцем)** | Независимый Challenger PASS на `ca3b23b` — см. 10.2; готовый комментарий ниже |
| #40 | Нет | Repo DONE, production E2E BLOCKED |
| #41 | Нет | Requirements open, не реализовано |
| #46 | Нет | Challenger TASK 7 + Main Re-Audit + production — всё OPEN |
| #50 | Нет | Producer-часть выполнена частично (этот файл ресинкнут, см. 10.4); Challenger + Main Re-Audit pending |

### 10.2 Независимый Challenger #36 (эта сессия, main @ `ca3b23b`)

Атаки на копии актуального main (`git archive ca3b23b`, node v22.22.3, embedded PG 18.4):

- **A.** Stray async crash в копии РЕАЛЬНОГО DB-набора (`availability-parity` + `setImmediate throw`
  перед `finishSuite`) → `ALL PASS`, **exit 1**. Канал A закрыт.
- **B.** `verify_app` против битого импорта (seam `VERIFY_APP_ENTRY`) →
  `IMPORT/LINK ERROR`, **exit 1**. Канал B закрыт.
- **D.** Все `tests/*.mjs` зарегистрированы в SUITES (независимый grep-цикл) — пропуск 0.
  Guard покрывает все SUITES (проверено чтением + зелёным guard в гейте).
- **E.** Фальсификация (`DEFAULT_DURATION_MIN` 60→45) → гейт красный, **exit 1**, 2 упавших
  набора. Гейт реагирует, не всегда-зелёный.
- **C2 (НОВАЯ находка → #51).** Silent-набор (0 проверок, `process.exit(0)`), корректно
  wired в SUITES → **гейт зелёный 25/25, exit 0**. Канал вне DoD #36 (объявлен
  out-of-scope в §5 отчёта Producer) — оформлен отдельным P2 #51, закрытие #36 не блокирует.
- Методологическая честность: первая проводка атаки C сломала синтаксис `verify_all`
  (не та форма записи SUITES) — результат отброшен как невалидный, атака повторена корректно.

**Вердикт: Challenger PASS.** Все три канала #36 закрыты на актуальном main.
#36 готова к закрытию владельцем.

### 10.3 Готовый комментарий для закрытия #36 (владельцем)

```markdown
## Независимый Challenger PASS (2026-09-25, main @ ca3b23b, сессия arena/01a0d700)

Все три канала перепроверены атаками на копии актуального main, а не перечитыванием отчёта:
- A: stray async crash в реальном DB-наборе → exit 1 (канал закрыт).
- B: verify_app против битого импорта → IMPORT/LINK ERROR, exit 1 (канал закрыт).
- C/D: guard покрывает все SUITES; все tests/*.mjs зарегистрированы.
- Фальсификация (DEFAULT_DURATION_MIN 60→45) → гейт красный, exit 1.

Новая находка того же класса (silent-набор с 0 проверок проходит зелёным) оформлена
отдельно как #51 и не входит в DoD этой задачи.

Закрываю. Evidence: docs/ISSUE-TRIAGE-2026-09-25.md §10.
```

### 10.4 Несложный фикс: ресинк `docs/CURRENT-STATE.md` (producer-часть #50)

Файл обновлён до `main @ ca3b23b`: PR #47/#48 merged, контурная таблица (#36 ready-to-close,
#51/#50 новые, #15/#49 закрыты), блокеры и next actions, футер синхронизации.
RECOVERY-ORCHESTRATION и ROADMAP-OKNA — остаток #50 (не тронуты в этом цикле).
Challenger + Main Re-Audit по #50 — pending (DoD #50).
