...
| **Оплата** | 3 платёжные ссылки (2× bePaid product, «свободный платёж») + блок банковских реквизитов (получатель, юр. адрес, УНП, р/с, банк, БИК, назначение платежа) — структура поддерживается, но конкретные реквизиты специалиста в репозитории не хранятся (заполняются в кабинете) |
...
└─ paymentRequisites   — банковские реквизиты (заполняются специалистом в кабинете)
...
Блок реквизитов плательщика (`payment_requisites`: получатель, юр. адрес, УНП, р/с, банк, БИК, назначение платежа) — часть публичного профиля каждого специалиста, редактируется в кабинете, публикуется на странице записи.

**Персональные данные.** Банковский счёт и регистрационные данные (ФИО ИП, юр. адрес, УНП, БИК) — персональные данные владельца кабинета: в сид-данных (`supabase/seed.sql`), демо-данных (`js/core/dbContext.js`), тестах и документации они не хранятся. Сид оставляет `payment_requisites` пустым (`'{}'::jsonb`), в тестах используются вымышленные значения. Реальные реквизиты вносит только сам специалист через кабинет, и они живут в базе.
