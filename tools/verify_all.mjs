...
const SUITES = [
  ...
  ['Telegram Mini App: initData HMAC, приватная привязка и Supabase session', 'node', ['tests/telegram-miniapp-auth.mjs']],
  ['Telegram registration: PostgreSQL RPC, private mapping, onboarding and public profile gate', 'node', ['tests/telegram-auth-db.mjs']],
  ['Регистрация специалиста через Google: SQL-контракт (поиск/привязка/дубли/RLS)', 'node', ['tests/google-specialist-signup.mjs']],
  ['Опрос в записи: маски, приоритет, manual-review для непокрытых векторов', 'node', ['tests/triage-routing.mjs']],
  ['Публичная оплата: нет клиентского пути «оплата прошла» (#21 п.4 → #121)', 'node', ['tests/demo-pay-honesty.mjs']],
  ['Issue #121 (R14): production UI без demo-записи/входа email+пароль/сброса; без сервера — недоступность', 'node', ['tests/production-ui-no-demo.mjs']],
  ...
  ['Issue #92: набор кабинета не зависит от дня недели (7 дат, инъекция часов)', 'node', ['tests/cabinet-weekday-matrix.mjs']],
  ['Авторизация: каналы кода, сессия, write-through', 'node', ['verify_auth.mjs']],
  ['UI входа специалиста: Google web + Telegram Mini App (OTP по email снят)', 'node', ['tests/auth-ui-pending.mjs']],
